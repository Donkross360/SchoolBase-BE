...
/**
 * Swagger decorators for Upload Audio endpoint
 */
export const ApiUploadAudio = () =>
  applyDecorators(
    ApiOperation({
      summary: 'Upload an audio file to MinIO',
      description:
        'Uploads an audio file (MP3, WAV, WebM, OGG, M4A) to MinIO and returns the URL. Maximum file size is 10MB. Only authenticated users can access this endpoint.',
    }),
    ApiConsumes('multipart/form-data'),
    ApiBody({
      schema: {
        ...
        properties: {
          file: {
            type: 'string',
            format: 'binary',
            description: 'Audio file to upload (MP3, WAV, WebM, OGG, or M4A, max 10MB)',
          },
        },
        required: ['file'],
      },
