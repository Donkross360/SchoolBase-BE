...
import { FeeStatus } from '../../fees/enums/fees.enums';
...
export class PaymentModelAction extends AbstractModelAction<Payment> {
  ...
  async getTotalCollected(
    ...
    // Only count payments that reference existing, active fee components
    // This prevents counting orphaned payments (payments for deleted fees)
    const query = this.feePaymentRepository
      .createQueryBuilder('payment')
      .innerJoin('payment.fee_component', 'fee_component')
      .select('COALESCE(SUM(payment.amount_paid), 0)', 'total')
      .where('payment.status = :status', { status: PaymentStatus.PAID })
      .andWhere('fee_component.status = :feeStatus', { feeStatus: FeeStatus.ACTIVE });
    ...
    // Debug logging to verify what's being counted
    console.log('[PaymentModelAction] getTotalCollected:', {
      termId,
      sessionId,
      total,
