...
export class ParentService {
  ...
  async create(createDto: CreateParentDto): Promise<ParentResponseDto> {
    ...
    try {
      ...
      // Log email error but don't throw - parent has already been created
      this.logger.warn(
        `Failed to send account creation email for parent ${response.id} (${response.email}). Parent account was created successfully.`,
        emailError,
      );
    }
  ...
  async getLinkedStudents(parentId: string): Promise<StudentBasicDto[]> {
    this.logger.info(`[getLinkedStudents] Fetching students for parent: ${parentId}`);
    ...
    this.logger.info(`[getLinkedStudents] Parent found: ${parentId}, querying students...`);
    ...
    this.logger.info(sysMsg.PARENT_STUDENTS_FETCHED, {
      ...
      studentIds: students.payload.map(s => s.id),
    });
  ...
  async getLinkedStudentProfileForParent(
    ...
    const { user, stream, current_class, class_assignments } = student;
    ...
    const activeClassAssignment = class_assignments?.find(ca => ca.is_active && ca.class) || null;
    const studentClass = current_class || activeClassAssignment?.class || stream?.class || null;
    ...
    return plainToInstance(
      StudentProfileDto,
      {
        ...
        photo_url: student.photo_url,
        class_subjects:
          studentClass?.classSubjects?.map((cs) => ({
            id: cs.id,
        ...
        timetable: studentClass?.timetable
          ? { id: studentClass.timetable.id }
          : null,
        class_details: studentClass
          ? { id: studentClass.id, name: studentClass.name }
          : null,
        academic_details: studentClass?.academicSession
          ? {
              id: studentClass.academicSession.id,
              session: studentClass.academicSession.name,
            }
          : null,
      },
      { excludeExtraneousValues: true },
