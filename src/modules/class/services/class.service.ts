...
export class ClassService {
  ...
  async getGroupedClasses(page = 1, limit = 20, includeArchived = false) {
    ...
    let classes = Array.isArray(classesRaw) ? classesRaw : [];

    // Manual filter as safety net to ensure correct filtering
    // Filter based on includeArchived flag
    if (!includeArchived) {
      // Show only active (non-archived) classes
      classes = classes.filter((cls) => !cls.is_deleted);
    } else {
      // Show only archived classes
      classes = classes.filter((cls) => cls.is_deleted === true);
    }
  ...
  /**
   * Reactivates a soft-deleted class by ID.
   * Sets is_deleted to false and deleted_at to null.
   */
  async reactivateClass(classId: string) {
    const classEntity = await this.classModelAction.get({
      identifierOptions: { id: classId },
      relations: { academicSession: true },
    });
    ...
    this.notifyClassUsers(
      classId,
      'Class Reactivated',
      `Your assigned class, ${classEntity.name} ${classEntity.arm || ''}, has been reactivated.`,
      EventAction.UPDATED,
    ).catch((err) =>
      this.logger.error(
        `Failed to notify users on class reactivation ${classId}`,
        err,
      ),
    );
    ...
    return {
      status_code: HttpStatus.OK,
      message: sysMsg.CLASS_REACTIVATED,
    };
  }
  ...
  async unassignStudentFromClass(
    ...
    // 2. Perform unassignment in transaction
    // Note: We don't validate student exists here because:
    // - The assignment existence check is sufficient
    // - Students might be soft-deleted but still have active assignments that need cleanup
    await this.dataSource.transaction(async (manager) => {
  ...
  /**
   * Assigns a teacher to a class as a class teacher
   */
  async assignTeacherToClass(
    ...
    // Get target session (default to active if not provided)
    const activeSession = await this.getActiveSession();
    const targetSessionId: string = sessionId || activeSession.id;

    // Check if assignment already exists
    const existingAssignment = await this.classTeacherModelAction.find({
      findOptions: {
        teacher: { id: teacherId },
        class: { id: classId },
    ...
    if (existingAssignment.payload.length > 0) {
      const assignment = existingAssignment.payload[0];
      // If assignment exists but is inactive, reactivate it
      if (!assignment.is_active) {
        await this.classTeacherModelAction.update({
          identifierOptions: { id: assignment.id },
          updatePayload: {
            is_active: true,
            assignment_date: new Date(),
          },
      ...
      throw new ConflictException(
        'Teacher is already assigned to this class for this session',
      );
    }
    ...
    this.logger.info(
      `Teacher ${teacherId} assigned to class ${classId} for session ${targetSessionId}`,
    );
