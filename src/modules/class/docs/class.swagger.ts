...
export const ClassSwagger = {
  ...
  endpoints: {
    ...
    reactivateClass: {
      operation: {
        summary: 'Reactivate a class (Admin)',
        description:
          'Reactivates a soft-deleted class by ID. Sets is_deleted to false and deleted_at to null.',
      },
      parameters: {
        id: {
          name: 'id',
          description: 'The Class ID to reactivate',
        },
      },
      responses: {
        ok: {
          status: HttpStatus.OK,
          description: sysMsg.CLASS_REACTIVATED,
          schema: {
            type: 'object',
            properties: {
              status_code: { type: 'integer', example: 200 },
              message: { type: 'string', example: sysMsg.CLASS_REACTIVATED },
            },
          },
        },
        badRequest: {
          status: HttpStatus.BAD_REQUEST,
          description: 'Class is already active',
          schema: {
            type: 'object',
            properties: {
              status_code: { type: 'integer', example: 400 },
              message: {
                type: 'string',
                example: 'Class is already active',
              },
            },
          },
        },
