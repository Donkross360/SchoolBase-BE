...
export class TeacherService {
  ...
  async create(createDto: CreateTeacherDto): Promise<TeacherResponseDto> {
    ...
    try {
      ...
      // Log email error but don't throw - teacher has already been created
      this.logger.warn(
        `Failed to send account creation email for teacher ${response.id} (${response.email}). Teacher account was created successfully.`,
        emailError,
      );
    }
