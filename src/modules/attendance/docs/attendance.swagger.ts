...
/**
 * Mark Attendance via NFC Card endpoint
 */
export const ApiMarkNfcAttendance = () =>
  applyDecorators(
    ApiOperation({
      summary: 'Mark attendance via NFC card scan (Mobile App)',
      description:
        'Mark student attendance by scanning their NFC card. ' +
        'The system looks up the student by NFC card ID or registration number, ' +
        'determines their current class, and marks them as PRESENT (or LATE if check-in is after 9 AM). ' +
        'If attendance is already marked for today, it updates the record if the status or time changed. ' +
        'This endpoint is designed for mobile apps using NFC readers on teacher phones.',
    }),
    ApiBody({
      type: MarkNfcAttendanceDto,
      description: 'NFC attendance marking payload',
      examples: {
        nfcScan: {
          summary: 'NFC Card Scan',
          value: {
            card_id: 'ABC123456789',
            date: '2025-12-23',
            check_in_time: '08:30:00',
            notes: 'Marked via NFC card',
          },
        },
        nfcScanToday: {
          summary: 'NFC Scan - Today (defaults)',
          value: {
            card_id: 'ABC123456789',
          },
        },
    ...
    ApiOkResponse({
      description: 'Attendance marked successfully via NFC',
      schema: {
        type: 'object',
        properties: {
          message: {
            type: 'string',
            example: 'Attendance marked successfully via NFC',
          },
          status_code: { type: 'number', example: 200 },
          data: {
            type: 'object',
            properties: {
              ...
              already_marked: {
                type: 'boolean',
                example: false,
                description: 'Whether attendance was already marked for today',
              },
            },
          },
        },
    ...
    ApiBadRequestResponse({
      description:
        'Invalid input, future date not allowed, or student not assigned to a class',
    }),
    ApiNotFoundResponse({
      description: 'Student not found with the provided card ID or registration number',
    }),
  );
