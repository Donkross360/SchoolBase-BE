...
import { IRequestWithUser } from 'src/common/types/request-with-user.interface';
...
export class StudentDailyAttendanceController {
  ...
  // --- POST: MARK ATTENDANCE VIA NFC CARD (TEACHER - MOBILE APP) ---
  @Post('nfc')
  ...
  async markNfcAttendance(
    @Req() req: IRequestWithUser,
    @Body() dto: MarkNfcAttendanceDto,
  ) {
    return this.attendanceService.markNfcAttendance(req.user.userId, dto);
  }
  ...
  async getClassDailyAttendance(
    ...
    // Pass userId for authorization check (teachers must be class teacher, admins bypass)
    return this.attendanceService.getClassDailyAttendance(
      classId,
      targetDate,
      req.user.userId,
    );
