...
export class AttendanceService {
  ...
  /**
   * Mark attendance via NFC card scan (for mobile app)
   * Looks up student by NFC card ID or registration number, then marks attendance
   */
  async markNfcAttendance(
    userId: string,
    dto: MarkNfcAttendanceDto,
  ): Promise<{
    message: string;
    status_code: number;
    data: {
    ...
    const { card_id, date, check_in_time, notes } = dto;

    // Determine attendance date (default to today)
    ...
    // Normalize card ID for case-insensitive lookup
    const { normalizeNfcCardId } = require('../../shared/utils');
    const normalizedCardId = normalizeNfcCardId(card_id);

    // Find student by NFC card ID (case-insensitive) or registration number
    // Use QueryBuilder for case-insensitive NFC lookup
    const student = await this.dataSource.manager
      .createQueryBuilder(Student, 'student')
      .leftJoinAndSelect('student.current_class', 'current_class')
      .leftJoinAndSelect('student.user', 'user')
      .where('(UPPER(student.nfc_card_id) = UPPER(:cardId) OR student.registration_number = :cardId)', {
    ...
    if (!student) {
      throw new NotFoundException(
        `Student not found with card ID or registration number: ${card_id}`,
      );
    }

    // Check if student has a class assigned
    if (!student.current_class_id || !student.current_class) {
      throw new BadRequestException(
        `Student ${student.registration_number} is not assigned to any class`,
      );
    }

    const classId = student.current_class_id;
    const className = student.current_class.name || 'Unknown Class';
    ...
    // Determine check-in time (default to current time)
    const now = new Date();
    const checkInTime = check_in_time || `${now.getHours().toString().padStart(2, '0')}:${now.getMinutes().toString().padStart(2, '0')}:${now.getSeconds().toString().padStart(2, '0')}`;

    // Check if attendance already exists for today
    const existingAttendance = await this.dataSource.manager.findOne(
    ...
    let finalStatus = DailyAttendanceStatus.PRESENT;

    // Determine if student is late (check-in after 9 AM)
    ...
    if (checkInDateTime > lateThreshold) {
      finalStatus = DailyAttendanceStatus.LATE;
    }

    await this.dataSource.transaction(async (manager) => {
      if (existingAttendance) {
        // Format existing check_in_time to HH:mm:ss for comparison
        let existingTimeStr = '';
        if (existingAttendance.check_in_time) {
          const existingTime = new Date(existingAttendance.check_in_time);
          existingTimeStr = `${existingTime.getHours().toString().padStart(2, '0')}:${existingTime.getMinutes().toString().padStart(2, '0')}:${existingTime.getSeconds().toString().padStart(2, '0')}`;
        }
        
        // Already marked - update if different status or time
        if (
          existingAttendance.status !== finalStatus ||
          existingTimeStr !== checkInTime
        ) {
          ...
          this.logger.info(
            `Updated NFC attendance for student ${student.registration_number} on ${attendanceDate.toISOString().split('T')[0]}`,
          );
        }
        alreadyMarked = true;
      } else {
        ...
        await this.studentDailyAttendanceModelAction.create({
          createPayload: {
            ...
            status: finalStatus,
            check_in_time: checkInTime,
            notes: notes || `Marked via NFC card (${card_id})`,
            marked_by: userId,
            marked_at: new Date(),
            is_locked: true, // Auto-lock NFC-based attendance
          },
          transactionOptions: {
            useTransaction: true,
            transaction: manager,
        ...
        this.logger.info(
          `Marked NFC attendance for student ${student.registration_number} on ${attendanceDate.toISOString().split('T')[0]} at ${checkInTime}`,
        );
      }
    });
    ...
    return {
      message: alreadyMarked
        ? 'Attendance already marked for today. Updated if needed.'
        : 'Attendance marked successfully via NFC',
      status_code: 200,
      data: {
        student_id: student.id,
  ...
  private async markScheduleBasedAttendance(
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      // Validate that this teacher is the class teacher for this class
      // Only class teachers can mark attendance (even for specific subjects/periods)
      const classTeacherAssignment = await manager.findOne(ClassTeacher, {
        where: {
          teacher: { id: teacherId },
          class: { id: classId },
          is_active: true,
  ...
  async getClassDailyAttendance(
    ...
    // If userId is provided (teacher viewing), validate they are the class teacher
    if (userId) {
      const teacher = await this.dataSource.manager.findOne(Teacher, {
        where: { user: { id: userId } },
      });
      ...
      if (teacher) {
        ...
        if (!classTeacherAssignment) {
          throw new ForbiddenException(
            'Only the class teacher can view attendance for this class',
          );
        }
      }
      // If user is not a teacher, they must be ADMIN (checked by role guard)
    }
